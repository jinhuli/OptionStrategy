...
class PortfolioStraddle(Portfolio):
    ...
    def __init__(self, evalDate, option_call, option_put, spot, strike, vol, rf, df_theory=None):
        ...
        self.metrics_call = OptionMetrics(option_call,rf,engineType).set_evaluation(self.evaluation)
        self.metrics_put = OptionMetrics(option_put,rf,engineType).set_evaluation(self.evaluation)
    ...
    def delta_neutral(self):
        iv_call = self.metrics_call.implied_vol(spot,self.metrics_call.Option.init_price)
        iv_put = self.metrics_put.implied_vol(spot,self.metrics_put.Option.init_price)
        ...
        self.ratio_put = rp*n
        p_put = self.metrics_put.option_price(spot, iv_put)
        p_call = self.metrics_call.option_price(spot, iv_call)
...
class PortfolioBackspread(Portfolio):
    ...
    def delta_neutral(self):
        ...
        p_long = self.metrics_long.option_price(spot, iv_long)
        p_short = self.metrics_short.option_price(spot, iv_short)
    ...
    def delta(self,implied_vol=None):
        if implied_vol == None:
            iv_long = self.metrics_long.implied_vol(spot, self.metrics_long.Option.init_price)
            iv_short = self.metrics_short.implied_vol(spot, self.metrics_short.Option.init_price)
        else:
