...
class OptionAccount(AbstractAccount):
    def __init__(self, init_fund: float = 1000000.0, contract_multiplier: int = 10000, fee_rate: float = 0.0 / 10000,
                 rf: float = 0.0, cash_reserve_pct: float = 0.2):
        ...
        self.trading_history: pd.DataFrame = None

    def init(self) -> None:
        cols = [
            Util.UUID, Util.ID_INSTRUMENT, Util.DT_TRADE, Util.TRADING_TYPE, Util.TRADING_PRICE, Util.TRADING_COST,
            Util.UNIT, Util.PREMIIUM_PAID, Util.CASH, Util.MARGIN_CAPITAL
        ]
        self.trading_history = pd.DataFrame(columns=cols)
        return None

    def add_record(self, trade_record: pd.Series) -> bool:
        # TODO validate
        self.trading_history.append(trade_record, ignore_index=True)
        premium = trade_record[Util.PREMIIUM_PAID]
        trading_cost = trade_record[Util.TRADING_COST]
        self.nbr_trade += 1
        self.cash -= premium + trading_cost
        self.total_transaction_cost += trading_cost
        return True
