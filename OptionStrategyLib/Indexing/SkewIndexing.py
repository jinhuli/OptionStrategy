...
class SkewIndexing(BktOptionStrategy):
    ...
    def get_T_quotes(self, df_mdt, eval_date):
        ...
        df = pd.merge(df_call[['amt_call_quote', self.util.col_applicable_strike]],
                      df_put[['amt_put_quote', self.util.col_applicable_strike]], how='inner', on=self.util.col_applicable_strike)
        ...
        df['amt_ttm'] = (maturitydt - eval_date).days / 365.0
        ...
        df.loc[0, 'amt_delta_k'] = delta_k
        df.loc[len(df) - 1, 'amt_delta_k'] = delta_k
    ...
    def forward_cboe(self, t_quotes, eval_date):
        ...
        k = df.sort_values(by='amt_cp_diff', ascending=True).index[0]
        p_call = df.loc[k, 'amt_call_quote']
        p_put = df.loc[k, 'amt_put_quote']
        F = df.loc[k, 'amt_fv'] * (p_call - p_put) + k
        df['k-f'] = df.index - F
    ...
    def for_calculation(self, df, eval_date):
        mid_k, K, F = self.forward_cboe(df, eval_date)
        df['atm_k'] = K
        df['mid_k'] = mid_k
        df['F'] = F
    ...
    def calculate_sigma_for_vix(self, df):
        ...
        fv = df.loc[0, 'amt_fv']
        v1 = (1.0 / T) * (F / K - 1) ** 2
        sigma = (2.0 / T) * fv * df['for_sigma'].sum() - v1
        return sigma
    ...
    def calculate(self, eval_date):
        ...
        NT1 = (mdt1 - eval_date).days * 1440
        NT2 = (mdt2 - eval_date).days * 1440
        N30 = 30 * 1400
        N365 = 265 * 1440
        w = (NT2 - N30) / (NT2 - NT1)
    ...
    def run(self):
        while self.bkt_optionset.index < len(self.bkt_optionset.dt_list) - 1:
            ...
            print("%20s %20s %20s" % (eval_date, vix, skew))
            self.bkt_optionset.next()


print('=' * 100)
print("%20s %20s %20s" % ('date', 'vix', 'skew'))
print('-' * 100)
