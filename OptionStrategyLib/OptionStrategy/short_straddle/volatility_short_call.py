...
while optionset.eval_date <= end_date:
    ...
    if not empty_position:
        moneyness_put = optionset.get_option_moneyness(atm_call)
    ...
    # Delta hedge
    if not empty_position :
        iv = atm_call.get_implied_vol()
        delta = atm_call.get_delta(iv)
        options_delta = unit_c * atm_call.multiplier() * delta
