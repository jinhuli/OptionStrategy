...
query_volume = sess2.query(optionMkt.dt_date, optionMkt.cd_option_type,optionMkt.id_underlying,
                           func.sum(optionMkt.amt_holding_volume).label('total_holding_volume'),
                           func.sum(optionMkt.amt_trading_volume).label('total_trading_volume')
                           ) \
    .filter(optionMkt.dt_date >= startDate) \
...
# 按持仓量最大选取主力合约
df = df[df.groupby(['dt_date','cd_option_type'])['total_holding_volume'].transform(max) == df['total_holding_volume']]
