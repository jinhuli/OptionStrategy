...
dc = DataCollection()
#####################################################################################
beg_date = datetime.date(2014, 12, 1)
end_date = datetime.date(2015, 1, 4)
# windcode = "000300.SH"
# id_instrument = 'index_300sh'
# db_data = dc.table_index_intraday().wind_data_equity_index(windcode, beg_date, end_date, id_instrument)
# try:
...
# if res.rowcount == 0:
    ...
    try:
        conn_intraday.execute(equity_index_intraday.insert(), db_data)
        print(dt_date,'equity_index_intraday-300sh -- inserted into data base succefully')
    except Exception as e:
        print(e)
# res = equity_index_intraday.select((equity_index_intraday.c.dt_datetime == dt_date + " 09:30:00") &
